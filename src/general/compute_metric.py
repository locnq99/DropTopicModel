...
def compute_perplexity(wordinds1, wordcnts1, wordinds2, wordcnts2,\
        num_topics, n_infer, alpha, beta, num_test):
    perplex = Stream(alpha, beta, num_topics, n_infer)
    LD = 0
    ld2 = []
    for i in range(num_test):
        ld = perplex.compute_perplexity(wordinds1[i], wordcnts1[i],\
...
def compute_npmi(beta, word_inv, num_docs, top_k = 10):
    """
    input:
    """
    # compute npmi score
